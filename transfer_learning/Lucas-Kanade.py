...
def optical_flow(old_frame, new_frame, window_size, min_quality=0.01):
    ...
    feature_list = cv2.goodFeaturesToTrack(
        old_frame, max_corners, min_quality, min_distance
    )
    ...
    ft = cv2.filter2D(new_frame, -1, kernel_t) - cv2.filter2D(
        old_frame, -1, kernel_t
    )  # Gradient over Time
    ...
    for feature in feature_list:  #   for every corner
        j, i = (
            feature.ravel()
        )  #   get cordinates of the corners (i,j). They are stored in the order j, i
...
def drawSeparately(old_frame, new_frame, U, V, output_file):
    ...
    displacement.fill(255.0)  # Fill the displacement plot with White background
    line_color = (0, 0, 0)
        ...
        for j in range(img2.shape[1]):
            ...
            if U[i][j] and V[i][j] and inRange(end_pixel, img1.shape):
                displacement = cv2.arrowedLine(
                    displacement, start_pixel, end_pixel, line_color, thickness=2
                )
...
if __name__ == "__main__":
    #   Read Input
    img1 = cv2.imread("../../Object-Tracking/Inputs/grove1.png")
    img1 = cv2.cvtColor(img1, cv2.COLOR_BGR2GRAY)
    print(img1)

    img2 = cv2.imread("../../Object-Tracking/Inputs/grove2.png")
    ...
    drawSeparately(img1, img2, U, V, "./Results/Grove_Seperate_Result.png")
